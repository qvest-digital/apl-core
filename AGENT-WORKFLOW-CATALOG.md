...
**A workload chart cannot emit a NetworkPolicy.** The team AppProject's
`namespaceResourceBlacklist` covers `NetworkPolicy`, `ResourceQuota`, `LimitRange` and Istio's
`VirtualService`, `Sidecar` and `Gateway`; `clusterResourceWhitelist` is empty. Environment netpols
go through `AplTeamNetworkControl` (`AGENT-ENVIRONMENTS.md` §7), never the chart.

This **corrects `research/catalogs-and-workloads.md`**, which cites the old `agentic-sdlc` chart as
evidence that a workload chart can ship a NetworkPolicy. It cannot. The platform's own netpols in
`team-reviews` all carry a tracking id from `team-reviews-team-ns-reviews`, which runs under project
`default` — a different, unrestricted door. Argo's own rejection has **not** been observed directly;
that test is folded into building the first chart.
...
**Teardown works — `prune: false` is not the blocker earlier notes claimed.** The workload
Application carries `finalizers: [resources-finalizer.argocd.argoproj.io]`, and its parent
`team-<id>-team-ns-<id>` app has `automated.prune: true`. So removing a workload prunes the child
Application, whose finalizer cascade-deletes its resources. `prune: false` only means "do not delete
resources that vanish from the manifest while the app still exists". Each link verified; the
end-to-end delete has not been watched.
...
1. **Does apl-api accept a team member's token and scope writes to that team?** If yes, the pipeline
   creates its own environment with an ordinary team credential. If no, environment creation also
   has to go through the admin side, which makes the broker considerably less narrow. Most
   load-bearing unknown, and one curl to answer.
2. **Does Argo actually refuse a workload chart's NetworkPolicy?** §7 reads the config; nobody has
   watched the sync fail. Falls out of building the first chart.
...
4. **Does deleting a workload actually delete its pods?** §7's chain says yes; nobody has watched it.
