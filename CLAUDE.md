...
| `AGENT-ENVIRONMENTS.md` | working notes on giving AI agents ephemeral environments: running a Turnstone node per environment, why it belongs in the `turnstone` namespace rather than a team's, per-node config as a tenancy lever, and what a netpol denial looks like from inside an agent | you are working on agent environments, Turnstone nodes, or anything in the agentic-platform direction — it is a record of what was proven live, not a design |
