  ...
  if (Number.isNaN(productId)) {
    ...
    if (process.env.DB_TYPE === 'mysql') {
      ...
      connection.connect(function(err) {
          ...
          connection.query('SELECT Rating FROM ratings', function (err, results) {
              if (err) {
      ...
      if (process.env.SERVICE_VERSION === 'v-faulty') {
        ...
        // in another half proceed as usual
        random = Math.random(); // returns [0,1] -- re-assigns the same function-scoped `var random` declared above
        if (random <= 0.5) {
