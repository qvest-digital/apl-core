...
**If you are here to bump Tekton Triggers: run `tools/bump-source.sh tekton <version>` from the
lab repo root and read the section "Bumping" below. Nothing under `manifests/` or `crds/` is tracked
anywhere: both are written at assemble time from the pins in `sources.lock`.**
...
upstream files, **byte for byte**, fetched at assemble time by the lab's `tools/fetch-sources.sh`
and verified against the sha256 pins in the lab's `sources.lock` (`manifests/SHA256SUMS` repeats
them in `sha256sum` format). A single template, `templates/manifests.yaml`, streams them through and
edits exactly three Deployments on the way. A bump is "move two pins"; the diff is upstream's diff.
...
extracted **mechanically and verbatim** by the lab's `tools/fetch-sources.sh` at assemble time, with
a header saying so. `templates/manifests.yaml` skips CRD documents in the stream, so each CRD object
exists once in the render (Helm `--include-crds` emits `crds/`), and the operator's pre-apply touches
the same objects. Never edit `crds/crds.yaml` by hand; it is regenerated on every assemble.

## Bumping

```bash
tools/bump-source.sh tekton v0.38.0           # in the lab repo: downloads both release files, writes their
                                              # sha256 pins and URLs into sources.lock; touches nothing else
```

The chart itself lives in `patches/apl-core/` (it replaces an upstream directory), the manifests do
not live anywhere. After the pin moved:

1. Bump `version`/`appVersion` in this `Chart.yaml` inside the tekton-triggers patch (fixup on the
   assembled tree, re-export the series with the `format-patch` line in `SYNC-UPSTREAM.md`).
   `tools/assemble.sh` refuses to continue while `appVersion` disagrees with `sources.lock`.
2. `task assemble`, then `helm template x build/apl-core/charts/tekton-triggers -n tekton-pipelines
   --include-crds` must succeed and contain three `kind: Deployment`. If upstream renamed a
   Deployment, the knob for it silently stops applying — the names are the keys of `$knobs` in the
   template; check them against `grep -A3 'kind: Deployment' manifests/release.yaml`.
